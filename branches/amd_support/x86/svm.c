...
{
	struct kvm_run *kvm_run = svm->vcpu.run;
	return handle_gp(&svm->vcpu, kvm_run);
}
...
static int handle_exit(struct kvm_vcpu *vcpu)
{
	...
	if (is_external_interrupt(svm->vmcb->control.exit_int_info) &&
	    ...
	    exit_code != SVM_EXIT_EXCP_BASE + GP_VECTOR &&		// HACK: Nitro triggers GPs
...
static void svm_set_gp_trap(struct kvm_vcpu *vcpu) {
	struct vcpu_svm *svm = to_svm(vcpu);
	svm->vmcb->control.intercept_exceptions |= 1 << GP_VECTOR;
	return;
}

static void svm_unset_gp_trap(struct kvm_vcpu *vcpu) { 
	struct vcpu_svm *svm = to_svm(vcpu);
	svm->vmcb->control.intercept_exceptions &= (u32)(~(1 << GP_VECTOR));
	return;
}
...
static struct kvm_x86_ops svm_x86_ops = {
	...
	.set_gp_trap = svm_set_gp_trap,
	.unset_gp_trap = svm_unset_gp_trap
};
