		       ...
		       unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_START_SCTRACE: {
		...
		r = start_syscall_trace(kvm, d.idt_index, d.syscall_reg, 0);
		break;
	}
	case KVM_STOP_SCTRACE: {
		r = stop_syscall_trace(kvm);
		break;
	}
	case KVM_START_SCMON: {
		struct kvm_vmi_data d;
		...
		r = scmon_start(kvm, d.idt_index, d.syscall_reg);
		break;
	}
	case KVM_STOP_SCMON: {
		r = scmon_stop(kvm);
	...
	case KVM_LIST_SCMON_RULES: {
		struct kvm_scmon_str str;

		r = -EFAULT;

		if (copy_from_user(&str, argp, sizeof str)) {
			printk("list_scmon_rules memory error\n");
			goto out;
		}

		r = scmon_list_rules(str.string, str.length);
		break;
	}
	case KVM_FLUSH_SCMON_RULES: {
		r = scmon_flush_rules();
		break;
	}
	case KVM_ADD_SCMON_RULE: {
		struct kvm_scmon_rule rule;

		r = -EFAULT;
		if (copy_from_user(&rule, argp, sizeof rule))
			goto out;

		r = scmon_add_rule(rule.cond_reg, rule.cond_val, rule.action_reg, rule.action_reg_offset, rule.action);
		break;
	}
	case KVM_REMOVE_SCMON_RULE: {
		unsigned int d;
...
int is_sysenter_sysreturn(struct kvm_vcpu *vcpu) {
