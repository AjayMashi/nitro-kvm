...
/* TODO: rewrite rule-handler/parser for performance, the current implementation
         uses WAY too many jumps -> many jumps & pipeline flushes */
...
int scmon_add_rule(enum kvm_reg cond_reg, unsigned long int cond_val, enum kvm_reg action_reg, int64_t action_reg_offset, enum scmon_action action)
{
	...
	struct actions *new_action;
	struct scmon_rule *current_rule, *rule = NULL;
	int extend_rule = -1;
	
	/* CF 2012-02-28, check if rule-condition already exists */
	current_rule = scmon_first_rule;
	while (current_rule != NULL && rule == NULL) {
		/* the trigger condition already exists, don't create a new rule*/
		if (current_rule->cond_reg == cond_reg && current_rule->cond_val == cond_val) {
			rule = current_rule;
		}
		extend_rule++;
	...
	new_action = kzalloc(sizeof(struct actions), GFP_KERNEL);
	if (new_action == NULL) {
		if (extend_rule == -1)
			kfree(rule);
		kfree(cond_reg_name);
	...
	if (rule->first_action != NULL) {
		rule->last_action->next = new_action;
		new_action->next = NULL;
		new_action->prev = rule->last_action;
	} else {
		rule->first_action = new_action;
		new_action->prev = NULL;
		new_action->next = NULL;
	}
	rule->last_action = new_action;
	
	new_action->action_reg = action_reg;
	new_action->action_reg_offset = action_reg_offset;
	...
	if (extend_rule == -1) {
		...
		printk("kvm:scmon: extended rule %d, adding clause: %s%+ld %s\n", extend_rule, action_reg_name, (long int) action_reg_offset, action_name);
	}
...
int scmon_delete_rule(unsigned int id) {
	...
	while (current_rule != NULL) {
		if (id == i) {
			...
			//remove each action from the actions list
			while (current_rule->first_action != NULL){
				current_action=current_rule->first_action;
				current_rule->first_action=current_action->next;
...
*/
	...
	if(current_rule->cond_reg == VCPU_SCMON_REGS_ANY){
		...
		snprintf(output_line, NITRO_SCMON_OUTPUT_LINE_MAX_SIZE-1, 
			"<rule><condition><rname>%s</rname><rval>0x%lX</rval></condition><actions>",cond_reg_name, current_rule->cond_val);
	}
}


static int snprint_action_register(char *action_value,
		enum kvm_reg action_reg, struct scmon_rule *current_rule, struct actions *current_action,
		/* memory allocated by scmon_print_trace: */
...
)
{
	...
	abs_offset = abs( current_action->action_reg_offset);

	if (current_action->action_reg_offset <= 0) {
		if (abs_offset > scmonactionreg) {
			printk("\nkvm:syscall_mon: offset is to big\n");
			return -1;
		}
	...
	//"%s%+ld %s = action_reg_name, (long int) current_action->action_reg_offset, action_name"
	...
	case SCMON_ACTION_HEX:
		snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>HEX</rform><rval>0x%lX</rval></reg>", action_reg_name, scmonactionreg);
		break;
	case SCMON_ACTION_INT:
		snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>INT</rform><rval>%ld</rval></reg>", action_reg_name, scmonactionreg);
		break;
	case SCMON_ACTION_UINT:
		snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>UINT</rform><rval>%lu</rval></reg>", action_reg_name, scmonactionreg);
		break;
	case SCMON_ACTION_DEREFHEX:
		kvm_read_guest_virt_system(scmonactionreg, &scmonderef, 8, vcpu, &error);
		snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>DREFHEX</rform><rval>0x%lX</rval></reg>", action_reg_name, scmonderef);
		break;
	case SCMON_ACTION_DEREFINT:
		kvm_read_guest_virt_system(scmonactionreg, &scmonderef, 8, vcpu, &error);
		snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>DREFINT</rform><rval>%ld</rval></reg>", action_reg_name, scmonderef);
		break;
	case SCMON_ACTION_DEREFUINT:
		kvm_read_guest_virt_system(scmonactionreg, &scmonderef, 8, vcpu, &error);
		snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>DREFUINT</rform><rval>%lu</rval></reg>", action_reg_name, scmonderef);
		break;
	case SCMON_ACTION_DEREFSTR:
		buffer = kmalloc(NITRO_SCMON_ACTION_VALUE_MAX_SIZE, GFP_KERNEL);
		if (buffer == NULL) {
			printk("\nkvm:syscall_mon: could not allocate memory for string buffer\n");
			return -1;
		}
		...
		len = snprintf(action_value, NITRO_SCMON_ACTION_VALUE_MAX_SIZE-1, "<reg><rname>%s</rname><rform>DEREFSTR</rform><rval>%s\n</rval></reg>", action_reg_name, buffer);
		kfree(buffer);
		break;
	}
...
int scmon_print_trace(char prefix, struct kvm_vcpu *vcpu) {
	...
	while (current_rule != NULL) {
		...
		if (rule_matches) {
			...
			snprint_action_register_headerline(output_line, current_rule, cond_reg_name, vcpu);
			snprintf(output_line, NITRO_SCMON_OUTPUT_LINE_MAX_SIZE - 1, "%s\0", output_line);
			//set the final byte to \0 to terminate the string in any case
			...
			//we need to do this for EACH action available!
			for (current_action = current_rule->first_action; current_action != NULL; current_action = current_action->next) {
				
				if (current_action->action_reg == VCPU_SCMON_REGS_ANY) {
					...
					for (i = 0; i < VCPU_REGS_RIP; ++i) {
						...
						if (snprint_action_register(action_value, all_registers[i], current_rule, current_action,
							cond_reg_name, action_reg_name, action_name, vcpu) != 0) {
							continue;
						}

						len = snprintf(output_line, NITRO_SCMON_OUTPUT_LINE_MAX_SIZE - 1, "%s\0", action_value);
						output_line[NITRO_SCMON_OUTPUT_LINE_MAX_SIZE - 1] = '\0';
						//Proc Output
						//nitro_output_append(output_line, NITRO_SCMON_OUTPUT_LINE_MAX_SIZE-1);
						printk(output_line);
					...
					if (snprint_action_register(action_value, current_action->action_reg, current_rule, current_action,
			...
			//Proc Output
			//nitro_output_append("</action_regs></syscall_mon>\n\0", NITRO_SCMON_OUTPUT_LINE_MAX_SIZE-1);
			printk("</actions></rule>");
...
int scmon_list_rules(char *buffer, unsigned int buffer_length) {
	...
	while (current_rule != NULL) {
		tmp[0] = '\0';
		printk("buffer_length == %d, local_buffer_length == %d\n", buffer_length, local_buffer_length);
		...
		scmon_register_to_name(current_rule->cond_reg, cond_reg_name);
		snprintf(tmp, 64, "[%d] if (%s == 0x%lX)\n", rule_index, cond_reg_name, current_rule->cond_val);
		local_buffer_length += strnlen(tmp, 64);
		strncat(local_buffer, tmp, 64);
		
		for (current_action = current_rule->first_action; current_action != NULL; current_action = current_action->next) {
			tmp[0] = '\0';
			scmon_register_to_name(current_action->action_reg, action_reg_name);
			scmon_action_to_name(current_action->action, action_name);

			snprintf(tmp, 64, "[%d] => %s%+ld %s\n", rule_index, action_reg_name, (long int) current_action->action_reg_offset, action_name);
			local_buffer_length += strnlen(tmp, 64);
			strncat(local_buffer, tmp, 64);
		}
		current_rule = current_rule->next;
