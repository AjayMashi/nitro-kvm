...
int htfu_unharden(struct kvm *kvm){
	printk(KERN_INFO "kvm:htfu: htfu_unharden \n");
...
void htfu_handle_gp(struct kvm_vcpu *vcpu, struct kvm_run *kvm_run){
	//printk(KERN_INFO "#GP caught\n");

	if(is_sysenter(vcpu) || is_sysexit(vcpu)){//sysenter/sysexit
		//printk(KERN_INFO "sysenter_sysreturn trapped\n");
		if (emulate_instruction(vcpu, 0, 0, 0) != EMULATE_DONE){
			printk(KERN_INFO "prob emulating sysenter_sysreturn\n");
			kvm_clear_exception_queue(vcpu);
			kvm_clear_interrupt_queue(vcpu);
			kvm_queue_exception_e(vcpu,GP_VECTOR,kvm_run->ex.error_code);
		}
	}
	else if(is_int(vcpu)){//int
		printk(KERN_INFO "int trapped\n");
		kvm_clear_exception_queue(vcpu);
		kvm_clear_interrupt_queue(vcpu);
